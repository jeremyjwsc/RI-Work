...
if os.path.exists('time_evolution_8graph.csv'):
    os.remove('time_evolution_8graph.csv')
...
def draw_plot(data):
    ...
    # write to csv file
    with open("time_evolution_8graph.csv", "a") as graph:
        global csv_x_index
...
def main():
    ...
    with open("final_8solution.csv", "w") as result:
        result.write(res)
        result.close()
    print("finished.")
    plt.show()
